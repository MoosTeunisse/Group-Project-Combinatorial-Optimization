...
def objective_aware_weights(inst):
    """make weights be based on relative importance of vehicle and distance costs"""
    ...
    alpha = 0.2 + 0.6 * distance_ratio
    beta = 0.2 + 0.6 * vehicle_ratio
...
def task_difficulty_score(inst, dist, task, max_load, max_depot_dist):
    ...
    alpha, beta = objective_aware_weights(inst)
