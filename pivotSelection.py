...
def find_task_and_depot_distance(inst, dist, task):
    """Return distance from depot to a task"""
    req = inst.Requests[abs(task) - 1]
...
def choose_pivot_from_route(inst, dist, route):
    """Given a route, choose one pivot task from that route"""
...
def more_objective_aware_weights(inst):
    """Make weights be based on relative importance of vehicle and distance cost, this can help when distance is less important"""
