...
def build_routes_parallel_day(inst, dist, task_list):
    """build route for each day using parallel regret"""
...
def build_routes_parallel_day_two_step(inst, dist, task_list):
    ...
    unrouted_tasks = [t for t in task_list if t not in pivots]
    
    while unrouted_tasks:
        best_insertion_per_task, second_best_insertion_per_task = scan_insertions(inst, dist, unrouted_tasks, routes)
        
        #check if routes match the number of unrouted tasks, if not add an empty route and restart
        if len(best_insertion_per_task) < len(unrouted_tasks):
            routes.append([])
            continue
        
        chosen_task = select_task(best_insertion_per_task, second_best_insertion_per_task, unrouted_tasks)
        
        chosen_r, chosen_p, chosen_cost = best_insertion_per_task[chosen_task]
        routes[chosen_r].insert(chosen_p, chosen_task)
        unrouted_tasks.remove(chosen_task)
    
    return [([0] + r + [0]) for r in routes]

def scan_insertions(inst, dist, unrouted, routes):
    """find the best and second best insertion for each unrouted task across all routes"""
    best_insertion_per_task = {}
    second_best_insertion_per_task = {}
...
def build_routes_parallel_regret(inst, delivery_day, dist):
    ...
    for day in sorted(day_tasks):
        day_routes[day] = build_routes_parallel_day(inst, dist, day_tasks[day])
...
def build_routes_parallel_regret_two_step(inst, delivery_day, dist):
    ...
    for day in sorted(day_tasks):
        day_routes[day] = build_routes_parallel_day_two_step(inst, dist, day_tasks[day])
